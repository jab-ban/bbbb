...
# ---------- Load CSVs from secrets ----------
try:
    senders_csv = st.secrets["SENDERS"]
    receivers_csv = st.secrets["REC"]

    senders_df = pd.read_csv(io.StringIO(senders_csv))
    receivers_df = pd.read_csv(io.StringIO(receivers_csv))
except Exception as e:
    st.error(f"Error reading data from secrets: {e}")
    st.stop()
...
if method == "Email":
    ...
    media_type = st.radio("Choose Media Type", options=["image", "document", "audio"], index=0, horizontal=True)
...
if st.button(f"Send {method} Messages"):
    ...
    st.success(f"✅ All done! {sent_count}/{total} messages processed successfully.")
